...
struct bt_uuid_16 bt_char_config   = BT_UUID_INIT_16(0xF400);
...
BT_GATT_SERVICE_DEFINE(
    ...
    BT_GATT_CHARACTERISTIC(&bt_char_config.uuid, BT_GATT_CHRC_WRITE, BT_GATT_PERM_WRITE, NULL, write_config, ct),
...
static ssize_t write_config(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
  uint8_t brightness;
  LOGI("Received config length = %d", len);
  ...
  memcpy(&brightness, buf, len);
  LOGI("New brightness = %d", brightness);

  if (brightness == 0) {
    oled_disable();
  } else {
    oled_set_contrast(brightness);
    oled_enable();
  }
